...
public:
	...
	float GetValue(unsigned long elapsedMillis) override final { return Calculate(elapsedMillis); }
	unsigned long GetElapsed() override { return static_cast<unsigned long> (millis() - StarTime); }
protected:
...
public:
	...
	float Calculate(unsigned long elapsedMillis) {
		if (elapsedMillis < Interval) return static_cast<float>(elapsedMillis) / Interval;
		return 1.0f;
	};
};
...
public:
	...
	float Calculate(unsigned long elapsedMillis) override {
		...
		if (elapsedMillis < _t0) return static_cast<float>(elapsedMillis) / _t0;
		else if (elapsedMillis < _t0 + _t1) return 1.0f;
		else return 1.0f - (static_cast<float>(elapsedMillis) - _t1 - _t0) / _t2;
	};
	unsigned long _t0; unsigned long _t1; unsigned long _t2;
};
...
public:
	...
	float Calculate(unsigned long elapsedMillis) override {
		...
		if (elapsed < _t0) return 0.0;
		if (elapsed < _t0 + _t1) return static_cast<float>(elapsed - _t0) / _t1;
		else if (elapsed < _t0 + _t1 + _t2) return 1.0f;
		else if (elapsed < _t0 + _t1 + _t2 + _t3) return 1.0f - (static_cast<float>(elapsed) - _t2 - _t1 - _t0) / _t3;
		return 0.0;
	};
	...
	unsigned long _t0; unsigned long _t1; unsigned long _t2; unsigned long _t3; unsigned long _t4;
};
#endif